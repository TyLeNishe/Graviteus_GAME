...
public class Time_Script : MonoBehaviour
{
    ...
    public Image clock;
    public Light Light;
    public static bool paused = false, weather_flag = false;
    ...
    void Update()
    {
        if (!paused)
        {
            ...
            if (hour >= 24f) { hour = 0.0f; weather_flag = true; OxygenManager.oxygen -= 1; } else { hour += Time.deltaTime * time_speed; day_cycle += Time.deltaTime * time_speed; }
